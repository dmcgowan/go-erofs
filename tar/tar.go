...
func buildFS(entries []parsed) *FS {
	...
	for _, e := range entries {
		...
		// Preserve children when a directory is overwritten by another
		// directory — tar archives may emit a directory header after
		// entries within that directory.
		if existing, ok := f.nodes[e.path]; ok {
			wasDir := existing.info.mode.IsDir()
			existing.info = e.info
			if !e.info.mode.IsDir() || !wasDir {
